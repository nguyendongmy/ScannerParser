...
Public Class Scanner1
    ...
    Private _SpectrumScanMinFreqThreshold As Hashtable
    Private _SpectrumScanMaxFreqThreshold As Hashtable
        ...
        Try
            Using sr As New StreamReader(_configFile)
                ...
                Do While sr.Peek() >= 0
                    ...
                    ElseIf strs(0).Trim.ToLower = "SpectrumScanMinimumFreqThresholdOutput_Default".ToLower Then
                        _SpectrumScanMinFreqThreshold.Add("Default", Convert.ToInt32(strs(1)))
                    ElseIf strs(0).Trim.ToLower.StartsWith("SpectrumScanMinimumFreqThresholdOutput_Band".ToLower) Then
                        Dim len As Integer = "SpectrumScanMinimumFreqThresholdOutput_Band".Length
                        Dim band As String = strs(0).Substring(len)
                        _SpectrumScanMinFreqThreshold.Add(band, Convert.ToInt32(strs(1)))
                    ElseIf strs(0).Trim.ToLower = "SpectrumScanMaximumFreqThresholdOutput_Default".ToLower Then
                        _SpectrumScanMaxFreqThreshold.Add("Default", Convert.ToInt32(strs(1)))
                    ElseIf strs(0).Trim.ToLower.StartsWith("SpectrumScanMaximumFreqThresholdOutput_Band".ToLower) Then
                        Dim len As Integer = "SpectrumScanMaximumFreqThresholdOutput_Band".Length
                        Dim band As String = strs(0).Substring(len)
                        _SpectrumScanMaxFreqThreshold.Add(band, Convert.ToInt32(strs(1)))
                    End If
    ...
    Private Function GetTimeOffset(ByVal FileName As String, ByVal FirstDateTime As DateTime) As Integer
        ...
        'DiffMin = Round(DateDiff(DateInterval.Minute, [FIRST record timestamp], [filename timestamp], 0)
        'TimeOffset = Int(DiffMin / 30 + 0.5) * 30 / 60
        '4. Then, for ALL values in "Time_Stamp" field, set [Time_Stamp] = [Time_Stamp] + TimeOffset

        Return TimeOffset
    End Function


    Public Sub Parse(ByVal EnableSIPDecoding As Boolean, ByVal TechType As String, ByVal fileInputObj As ScannerFileInputObject, ByVal MarketName As String, ByVal Campaign As String, ByVal ClientName As String)
        ...
        _CollectedDateFormat = Convert.ToDateTime(_CollectedDate).ToString("dd/MM/yyyy", CultureInfo.InvariantCulture)
        ...
        '    outputFileName = filePath + "\" + _FileName + "_" + TechType + ".txt"
        'End If
        If TechType <> "SPECTRUMSCAN" Then
        ...
        If fileInputObj.FileMergeName.Trim <> "" And Not fileInputObj.isSkip Then
            If fileInputObj.FileMergeName.Trim <> "" And Not fileInputObj.isSkip Then
                For i = 0 To _DataRowList.Count - 1
                    ...
                    dttmFormat = Convert.ToDateTime(dttm).ToString("dd/MM/yyyy HH:mm:ss", CultureInfo.InvariantCulture)
                            ...
                            For j2 = 0 To NumRfDataBlock - 1
                                ...
                                With .RefScan(j)
                                    ...
                                    Try
                                        ...
                                        If .RP(j2) <> -999 Then
                                            Dim channelFoundStr = "(" + myBand.ToString + ":" + channel.ToString + ")"
                ...
                If type.ToUpper() = "CDMA" Then
                    ...
                    If CDMAPilotInfo.NumRecords = 0 Then
                        ...
                        If ch <> -999 Then
                            ...
                            Dim SIDNIDList As List(Of SIDNIDStore) = CDMAChannelList(ch)
            ...
            Case 10104
                ...
                If type.ToUpper() = "CDMA" Then
                    ...
                    If SID.Trim <> "" Then
                        ...
                        Dim SIDNIDList As List(Of SIDNIDStore) = CDMAChannelList(CDMAPilotInfo.Channel)
                        If SIDNIDList.Count > 1 Then
                            For j = 0 To SIDNIDList.Count - 1
                        ...
                        CDMAChannelList(CDMAPilotInfo.Channel) = SIDNIDList
            ...
            If msgType <> 10011 Or GSM_BCCHInfo.NumChannels = Count_GSMRecord + 1 Then
                ...
                If _isOutputLTE Then
                    ...
                    For i = 0 To operators.Count - 1
                        ...
                        If Not (tmpFileWriter.isDoNotExist And _ScanID <> _LastScanID) Then

                            If currentLTEChannelData.OperatorName <> operators(i) Then
                                If _RECaddNoSCondition.lastScanId <> _ScanID Then
                                    tmpFileWriter.countNoOperator = tmpFileWriter.countNoOperator + 1
                                End If
                            Else
                            ...
                            If currentLTEChannelData.OperatorName = operators(i) Then
                                tmpFileWriter.currentNosData.OperatorName = currentLTEChannelData.OperatorName
                                tmpFileWriter.currentNosData.prev_lat = lat
                                tmpFileWriter.currentNosData.prev_lon = lon
                                tmpFileWriter.currentNosData.Band = currentLTEChannelData.Band
                                tmpFileWriter.currentNosData.LogTrace = LogTrace
                                tmpFileWriter.currentNosData.Channel = currentLTEChannelData.channel
                                tmpFileWriter.currentNosData.MCC = MCC
                                tmpFileWriter.currentNosData.MNC = MNC

                                If _currentRSSIValue = -999 Then
                                    If _RECaddNoSCondition.lastScanId <> _ScanID Then
                                        tmpFileWriter.countRSSINev999 = tmpFileWriter.countRSSINev999 + 1
                                        _canWrite = False
                                    End If
                                Else
                                ...
                                If tmpFileWriter.countRSSINev999 > _Threshold_RSSI Or tmpFileWriter.countNoOperator > _Threshold_Operator Then
                                    If Not IsNothing(tmpFileWriter.currentNosData.LogTrace) And tmpFileWriter.currentNosData.LogTrace.Trim <> "" Then
                                        tmpFileWriter.currentNosData.LogTrace = tmpFileWriter.currentNosData.LogTrace + "|"
                                    End If

                                    blankPolygonLTE = msgType & "," & tmpFileWriter.currentNosData.prev_Time_Stamp & "," & tmpFileWriter.currentNosData.prev_GPS_Time & "," & tmpFileWriter.currentNosData.prev_lat & "," & tmpFileWriter.currentNosData.prev_lon & ",LTE," & tmpFileWriter.currentNosData.prev_ScanId & "," & tmpFileWriter.currentNosData.Channel & "," & tmpFileWriter.currentNosData.Band & "," & tmpFileWriter.currentNosData.CarrierBW &
                                                  ",-130,-999,-130,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999," &
                                                   tmpFileWriter.currentNosData.OperatorName & ",ALL," & _FileName & "," & _CollectedDate & "," & _MarketName & "," & _Campaign & "," & _ClientName & "," + tmpFileWriter.currentNosData.LogTrace + "ADDrecNoS,,," & tmpFileWriter.currentNosData.MCC & "," & tmpFileWriter.currentNosData.MNC & ",,,,,,,,,"

                                    If tmpFileWriter.countRSSINev999 > _Threshold_RSSI Then

                                        _TempStringList.Add(blankPolygonLTE)


                                        Dim _TempStringList1 As New List(Of String)

                                        For Each myStr_loop In _TempStringList
                                            Dim strs = myStr_loop.Split(",")
                                            If strs(53) = tmpFileWriter.OperatorName Then
                                ...
                                tmpFileWriter.currentNosData.CarrierBW = _currentNosData.CarrierBW
                                tmpFileWriter.currentNosData.prev_lat = lat
                                tmpFileWriter.currentNosData.prev_lon = lon
                                tmpFileWriter.currentNosData.prev_Time_Stamp = dttm
                                tmpFileWriter.currentNosData.prev_GPS_Time = dttm
                                tmpFileWriter.currentNosData.prev_ScanId = _ScanID
                                _OperatorSet.Item(operators(i)) = tmpFileWriter
                            End If
                        End If
                        If tmpFileWriter.countRSSINev999 = 0 Then
                    ...
                    If count_temp = operators.Count Or _TempStringList.Count > 5000 Then
                        _canWrite = True
                    End If
lineoutputLTE:
                    ...
                    If isLast Or _canWrite Then
                        ...
                        For Each myStr In _TempStringList
                            ...
                            If (Convert.ToDateTime(strs(1)) - _lastEndRoundTimeStamp).TotalSeconds > 5 Then
                                isTimeLimitReached = True

                            End If

                            If (Convert.ToDateTime(strs(1)) - _lastEndRoundTimeStamp).TotalSeconds > 2 And ((_LTEScannerData.channelRepeatedCount3030 > Int(count_input_chanel3030 * 2.5) - 1 And _isNo3040Message) Or (_LTEScannerData.channelRepeatedCount3040 > Int(count_input_chanel3040 * 2.5) - 1) And _isNo3030Message) Then
                                isTimeLimitReached = True
                                ...
                                If Not IsNothing(_OperatorBandSet.Item(key)) And Not isWriteBandOperator Then
                                    If (_OperatorBandSet.ContainsKey(key)) Then
                                        ...
                                        If Not tmpFileWriter.isDoNotExist Then
                                            ...
                                            End If
                                            tmpFileWriter.writer.WriteLine(myStr)
                                            _OperatorBandSet.Item(currentOperatorName) = tmpFileWriter
                                        End If
                                    End If
                                End If
                ...
                If _isOutputWCDMA Then
                    ...
                    For i = 0 To operators.Count - 1
                        ...
                        If Not (tmpFileWriter.isDoNotExist And _ScanID <> _LastScanID) Then
                            ...
                            If currentWCDMAChannelDaTa.OperatorName <> operators(i) Then
                                If _RECaddNoSCondition.lastScanId <> _ScanID Then
                                    tmpFileWriter.countNoOperator = tmpFileWriter.countNoOperator + 1
                                End If
                            Else
                            ...
                            If currentWCDMAChannelDaTa.OperatorName = operators(i) Then
                                tmpFileWriter.currentNosData.OperatorName = currentWCDMAChannelDaTa.OperatorName
                                tmpFileWriter.currentNosData.prev_lat = lat
                                tmpFileWriter.currentNosData.prev_lon = lon
                                tmpFileWriter.currentNosData.Band = currentWCDMAChannelDaTa.Band

                                tmpFileWriter.currentNosData.Channel = currentWCDMAChannelDaTa.channel
                                tmpFileWriter.currentNosData.MCC = currentWCDMAChannelDaTa.SID
                                tmpFileWriter.currentNosData.MNC = currentWCDMAChannelDaTa.NID
                                ...
                                If tmpFileWriter.countRSSINev999 > _Threshold_RSSI Or tmpFileWriter.countNoOperator > _Threshold_Operator Then
                                    blankPolygonUMTS = "10103," & tmpFileWriter.currentNosData.prev_Time_Stamp & "," & tmpFileWriter.currentNosData.prev_GPS_Time & "," & tmpFileWriter.currentNosData.prev_lat & "," & tmpFileWriter.currentNosData.prev_lon & "," & tmpFileWriter.currentNosData.Band & ",UMTS," & tmpFileWriter.currentNosData.prev_ScanId & "," & tmpFileWriter.currentNosData.Channel & "," & tmpFileWriter.currentNosData.MCC & "," & tmpFileWriter.currentNosData.MNC & ",-130,-999,-999,-999,-999,-999,-130,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999," & operators(i) & ",ALL," & tmpFileWriter.currentNosData.FileName & "," & _CollectedDate & "," & _MarketName & "," & _Campaign & "," & _ClientName & ",ADDrecNoS,,,,,,"

                                    If tmpFileWriter.countRSSINev999 > _Threshold_RSSI Then
                                        _TempStringList.Add(blankPolygonUMTS)
                                        ...
                                        For Each myStr_loop In _TempStringList
                                            ...
                                            If strs(66) = tmpFileWriter.OperatorName Then
                                ...
                                'Band, Channel, MCC, MNC, Operator, FileName 

                                tmpFileWriter.currentNosData.Band = _currentNosData.Band
                                tmpFileWriter.currentNosData.Channel = _currentNosData.Channel
                                tmpFileWriter.currentNosData.MCC = _currentNosData.MCC
                                tmpFileWriter.currentNosData.MNC = _currentNosData.MNC

                                tmpFileWriter.currentNosData.FileName = _FileName
                                tmpFileWriter.currentNosData.prev_lat = lat
                                tmpFileWriter.currentNosData.prev_lon = lon
                                tmpFileWriter.currentNosData.prev_Time_Stamp = dttm
                                tmpFileWriter.currentNosData.prev_GPS_Time = dttm
                ...
                If (_isOutputCDMA) Then
                    ...
                    For i = 0 To operators.Count - 1
                        ...
                        If Not (tmpFileWriter.isDoNotExist And _ScanID <> _LastScanID) Then
                            ...
                            If currentCDMAChannelData.OperatorName <> operators(i) Then
                                If _RECaddNoSCondition.lastScanId <> _ScanID Then
                                    tmpFileWriter.countNoOperator = tmpFileWriter.countNoOperator + 1
                                End If
                            Else
                            ...
                            If currentCDMAChannelData.OperatorName = operators(i) Then
                                tmpFileWriter.currentNosData.OperatorName = currentCDMAChannelData.OperatorName
                                tmpFileWriter.currentNosData.prev_lat = lat
                                tmpFileWriter.currentNosData.prev_lon = lon
                                tmpFileWriter.currentNosData.Band = currentCDMAChannelData.Band
                                tmpFileWriter.currentNosData.Channel = currentCDMAChannelData.channel
                                tmpFileWriter.currentNosData.MCC = currentCDMAChannelData.SID
                                tmpFileWriter.currentNosData.MNC = currentCDMAChannelData.NID
                                ...
                                If tmpFileWriter.countRSSINev999 > _Threshold_RSSI Or tmpFileWriter.countNoOperator > _Threshold_Operator Then
                                    blankPolygonCDMA = "10103," & tmpFileWriter.currentNosData.prev_Time_Stamp & "," & tmpFileWriter.currentNosData.prev_GPS_Time & "," & tmpFileWriter.currentNosData.prev_lat & "," & tmpFileWriter.currentNosData.prev_lon & "," & tmpFileWriter.currentNosData.Band & ",CDMA," & tmpFileWriter.currentNosData.prev_ScanId & "," & tmpFileWriter.currentNosData.MCC & "," & tmpFileWriter.currentNosData.MNC & ",-130,-999,-999,-130,-4-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999," & operators(i) & ",ALL," & tmpFileWriter.currentNosData.FileName & "," & _CollectedDate & "," & _MarketName & "," & _Campaign & "," & _ClientName & ",ADDrecNoS"
                                    ...
                                    If tmpFileWriter.countRSSINev999 > _Threshold_RSSI Then
                                        ...
                                        For Each myStr_loop In _TempStringList
                                            ...
                                            If strs(54) = tmpFileWriter.OperatorName Then
...
lineoutput:
                    If _canWrite Or isLast Then
                        ...
                        For Each myStr In _TempStringList
                            ...
                            For i = 0 To operators.Count - 1
                                ...
                                If tmpFileWriter.isDoNotExist Then
                                    If Not _ScanIDSet.Contains(_ScanID.ToString() + tmpFileWriter.OperatorName) Then
                                        ...
                                        blankPolygonCDMAMAX = "10103," & _FirstScanIDData.time_stamp & "," & _FirstScanIDData.gps_time_stamp & "," & _FirstScanIDData.lat & "," & _FirstScanIDData.lon & ",-999,CDMA," & _ScanID & ",-999,-999,-999,-130,-999,-999,-130,-999,-999,-130,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999,-999," & tmpFileWriter.OperatorName & ",MAX," & _FileName & "," & _CollectedDate & "," & _MarketName & "," & _Campaign & "," & _ClientName & ",ADDrecNoS"
                            ...
                            If _CDMAScannerData.inputChannelList.Contains(strs(8)) Then
                                _CDMAScannerData.inputChannelList.Remove(strs(8))
        ...
        If MessageType = "10103" Then
            ...
            If (Not IsNothing(strMsgData)) Then
                If (Not IsNothing(strMsgData(2))) Then
                    ...
                    If strMsgData(8).Trim <> "100" Then
                        'UMTS channel
                        'Dim tmpStr As String = "(" + _myBand.ToString() + ":" + channel.ToString() + ")"

                        'If _WCDMAChannelFound.IndexOf(tmpStr) < 0 Then
                        '    _WCDMAChannelFound = _WCDMAChannelFound + tmpStr
                        'End If
                    ...
                    Else
                        ...
                        'If _CDMAChannelFound.IndexOf(tmpStr) < 0 Then
                        '    _CDMAChannelFound = _CDMAChannelFound + tmpStr
                        'End If
                        _isSkippedCDMA = False
                        _isSkippedWCDMA = True
                    ...
                    If Integer.TryParse(channel.Trim, tmpInt) And TechType = "CDMA" Then
                        If tmpInt > 0 And Not CDMAChannelList.ContainsKey(channel) Then
                            Dim SIDNID As New SIDNIDStore()
                            Dim SIDNIDList = New List(Of SIDNIDStore)

                            CDMAChannelList.Add(channel, SIDNIDList)
                            _CDMAScannerData.inputChannelList.Add(channel)
                            CDMAChannelListWithTime.Add(channel, ChannelStore)
                        End If

                        If CDMAChannelListWithTime.ContainsKey(channel) Then
                            ChannelStore = CDMAChannelListWithTime.Item(channel)
                            ...
                            CDMAChannelListWithTime.Item(channel) = ChannelStore
                        End If
            ...
            Try
                If (Not IsNothing(strMsgData)) Then
                    ...
                    If SIDNID.SID.Trim <> "" And SIDNID.NID.Trim <> "" And SIDNID.SID.Trim.Length + SIDNID.SID.Trim.Length > 1 Then
                        If TechType = "CDMA" Then
                            If CDMAChannelList.ContainsKey(currentCDMAChannel) Then
                                If Not CDMASIDHashSet.Contains(SIDNID.SID.Trim + currentCDMAChannel.Trim) Then
                                    ...
                                    CDMAChannelList.Item(currentCDMAChannel).Add(SIDNID)
                                End If
            ...
            If tmpList.Count > 1 Then
                For i = 0 To tmpList.Count - 2
                    ...
                    If tt > 5 Or tt < -5 Then
                        inputChannelObj = tmpList(i)
                        inputChannelObj.isStartLate = True
                        _CDMAScannerData.inputChannelList.Remove(inputChannelObj.channel)
    ...
    Private Sub GetDoNotExistList(ByVal fileName As String, ByVal TechType As String)
        ...
        Dim FileNameOnly As String = fileName.Substring(fileName.LastIndexOf("\") + 1)
        Dim DoNotExistList As New List(Of String)
        ...
        If TechType = "LTE" Then
            ...
            If DoNotExistList.Count > 0 Then
                ...
                For Each OpName In DoNotExistList
